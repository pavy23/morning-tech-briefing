...
정상일 때 보통 몇 개가 살아남는지 로그로 확인한 뒤, 그 절반 아래로 두는 것을 권장합니다.
...
> `groundingMetadata` 없음, 실제 링크 20건 중 1~4건). 그래서 `gemini-3*` 모델에는 JSON 대신 라벨이 붙은
> 번호 목록으로 받고, 각 항목의 출처는 `groundingSupports`에서 코드로 붙입니다. 모델이 URL을 직접 쓰지
> 않습니다. `NEWS_FORMAT=json|list`로 자동 선택을 덮어쓸 수 있습니다.
>
> 참고: thinking 토큰도 `maxOutputTokens`에 포함됩니다. 2.5-flash는 약 2,700, 3.6-flash는 약 7,000~9,000토큰을
> 쓰므로 `src/fetch-news.mjs`의 한도는 **32768**입니다. 이보다 훨씬 낮으면 출력이 잘려 뉴스가 몇 개만 나옵니다.
...
**"Gemini API 503"**
- Gemini 서버 일시 과부하(transient). `fetch-news.mjs`가 기본 모델의 일시 오류(429/5xx) 사이에 **20초, 40초**를 기다리고, 3회째 막히면 **백업 모델(`gemini-3.5-flash-lite`)로 전환**해 발송 누락을 막음. 가벼운 모델은 긴 출력이 잘리므로 백업에는 후보 전체가 아니라 `total`건만 요청하고, 같은 그라운딩 가드를 적용함. 백업 모델 자체가 실패하면(잘못된 ID, 할당량 0) 남은 시도는 기본 모델로 복귀. 총 6회 시도
...
- 6회 재시도 후에도 실패해 그대로 발송됐다면 `재시도 후에도 실제 기사 링크 N개뿐` 경고가 남음 → 수동 재실행
