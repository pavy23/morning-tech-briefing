...
### 🧪 TypeSafe(Jev) 섀도 판정 — 메일은 그대로, 판정만 기록
...
현재는 **섀도 모드**라 메일 내용과 순서는 바뀌지 않고, 판정 결과만 로그와 아티팩트에 남습니다.
...
- Secrets에 `TYPESAFE_API_KEY`가 있을 때만 실행. 없거나 실패하면 건너뛰고 **메일 발송에는 영향 없음**
- 하루 요청 21회(중복 1 + 항목별 2×10), 입력 약 2만 토큰 이하 → 공개 단가 기준 월 수 센트
- Actions 로그에 `[shadow]` 항목별 판정과 **제안 순서**가 찍히고, `out/shadow-YYYY-MM-DD.json`이
  실행 아티팩트(`shadow-report-*`, 90일 보관)로 올라감
- 전날 헤드라인 이력은 `.briefing-state/history.json`에 두고 Actions 캐시로 하루 단위로 이어 받음
  (저장소에 커밋하지 않음)
- 리포트에서 중복/재탕으로 표시할 확률 기준은 `DUP_THRESHOLD`, `REPEAT_THRESHOLD` (기본 0.7)

**테스트**: Actions 탭 → Run workflow → `skip_email` 체크 → 메일 없이 수집·판정 로그만 확인.
...
`summary.md`(재탕·중복 감지 목록, 정답지 대조, 날짜별 관련도 상위 3), `grading.csv`(직접 채점용 시트),
`results.json`을 올립니다. 메일 발송과 Gemini 호출은 없고 Jev 비용은 90일 기준 0.1달러 미만입니다.
로그에는 헤드라인·카테고리만 있어 실제 운영보다 정보가 적으므로 결과는 하한선으로 봅니다.

**다음 단계**: 백테스트와 2주치 리포트를 보고 제안 순서가 실제 순서보다 나은지 확인되면, 20건 과수집 → Jev 판정 →
...
- Google 검색 그라운딩이 빠진 채 모델이 지어낸 배치. 로그에서 `실제 기사 링크 0/10` 같은 줄과
