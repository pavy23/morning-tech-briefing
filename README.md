...
src/index.mjs 실행  (주제·카테고리·색상은 briefing.config.json에서 읽음)
        ...
        ├─ judge.mjs        → TypeSafe(Jev)로 재탕·중복·관련도·카테고리 판정
        ├─ select.mjs       → 재탕·중복 제거 후 카테고리 배분대로 10개 선별
...
### 🧪 TypeSafe(Jev) 판정과 선별 — 재탕·중복은 빼고, 순서는 아직 그대로
...
재탕·중복 제거 → 설정의 카테고리 배분(예: AI 3, XR 2, 우주 2, 로봇 3)대로 채움 → 남으면 순서대로 보충.
순서는 Gemini 중요도 순을 유지합니다. 관련도 정렬은 독자가 채점해 확인한 뒤 켤 예정입니다.

근거: 87일치 백테스트에서 발송 항목의 10.1%가 이미 다룬 사건이었고(같은 사건이 최대 6일 반복),
한 메일에 같은 헤드라인이 두 번 실린 날도 있었습니다. 재탕 확률은 양극화되어(75%가 0.2 미만, 8%가 0.8 이상)
0.7 기준이 안정적이며, 0.3~0.7 회색지대(9%)는 제거하지 않습니다.
...
- 제거 기준 확률은 `DUP_THRESHOLD`, `REPEAT_THRESHOLD` (기본 0.7)
...
그 뒤 후보로 "사건 없는 일반론 걸러내기"와 "같은 논쟁 묶기" 판정을 백테스트로 검증한 후 추가합니다. 판정 질문은 `src/judge.mjs`에, 독자 프로필은 Secret `READER_PROFILE`에 있습니다.
